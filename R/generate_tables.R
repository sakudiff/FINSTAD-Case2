...
summary_data <- data.frame(
  Ticker = c("TEL", "MER", "AEV", "NVDA", "META", "BTC", "SPY", "PH 91D T-Bill"),
  ...
  Market_Exchange = c("PSE / NYSE (PHI)", "PSE / OTC (MAEOY)", "PSE / OTC (ABTZY)",
                      "NASDAQ", "NASDAQ", "Global Crypto", "NYSE Arca", "BSP / Bureau of Treasury"),
  Observations = c(rep("1,509 Trading Days", 7), "1,509 Trading Days"),
  Sample_Period = c(rep("Jan 2, 2020 -- Dec 31, 2025", 8))
)
...
# -------------------------------------------------------------
stats_mat <- table.Stats(returns_xts)
stats_df <- as.data.frame(stats_mat) %>%
  rownames_to_column(var = "Metric")

# Filter key statistical rows
key_metrics <- c("Observations", "Mean", "Std Dev", "Skewness", "Kurtosis", "Minimum", "Maximum")
...
# -------------------------------------------------------------
cor_mat <- cor(returns_xts, use = "pairwise.complete.obs")
