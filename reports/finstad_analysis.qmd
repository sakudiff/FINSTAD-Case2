...
```{r}
# --- Efficient Frontier: sweep target returns, solve exact QP at each one ---
# This is a quadratic program (quadratic objective, linear constraints) with
# one unique global optimum per target -- nloptr/SLSQP with numerical
# (finite-difference) gradients was landing on feasible-but-suboptimal
# points for some targets, producing the sawtooth. Solving the exact QP via
# quadprog removes gradient approximation error entirely.
library(quadprog)

target_returns <- seq(min(mu_ann), max(mu_ann), length.out = 50)
frontier_list  <- vector("list", length(target_returns))

# solve.QP minimizes 0.5*w'Dw - d'w subject to t(Amat)%*%w >= bvec,
# with the first `meq` constraints treated as equalities.
D <- 2 * Sigma_ann + diag(1e-6, n_assets)  # was 1e-8
d <- rep(0, n_assets)
...
# Helper: true close-to-close cumulative return per asset over a window,
# compounded from the carried simple-return series (ret_smp). Since ret_smp
# has no NA gaps, this captures the full move instead of skipping gap days
# (unlike the previous log-return + na.rm approach, which understated
# MER/AEV shocks by skipping the exact days the price gap happened on).
...
# Assemble stress_scenarios from real computed values
