...
# Build a single simple-return basis from the carried (gap-filled) log-return
# series, consistent with the rest of the report — Portfolio Construction also
# converts carried log returns to simple returns before Return.portfolio.
ret_smp <- exp(returns_xts_carried) - 1

# Annualize mean returns and covariance matrix from this shared basis
mu_ann    <- colMeans(ret_smp, na.rm = TRUE) * 252
Sigma_ann <- cov(ret_smp, use = "pairwise.complete.obs") * 252
...
# Uses ret_smp so this shares the same basis as mu_ann/Sigma_ann above.
